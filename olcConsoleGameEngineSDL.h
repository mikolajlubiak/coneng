...
your actions. However this code is protected by GNU GPLv3, see the license in
the github repo. This means you must attribute me if you use it. You can view
this license here: https://github.com/OneLoneCoder/videos/blob/master/LICENSE

Cheers!

Background
~~~~~~~~~~
If you've seen any of my videos - I like to do things using the windows console.
It's quick and easy, and allows you to focus on just the code that matters -
ideal when you're experimenting. Thing is, I have to keep doing the same
initialisation and display code each time, so this class wraps that up.
...
Usage:
~~~~~~
This class is abstract, so you must inherit from it. Override the OnUserCreate()
function with all the stuff you need for your application (for thready reasons
it's best to do this in this function and not your class constructor). Override
the OnUserUpdate(float fElapsedTime) function with the good stuff, it gives you
the elapsed time since the last call so you can modify your stuff dynamically.
Both functions should return true, unless you need the application to close.
...
The draw routines treat characters like pixels. By default they are set to white
solid blocks - but you can draw any unicode character, using any of the colours
listed below.
...
#else
    std::ofstream f((char *)sFile.c_str(), std::ios::out | std::ios::binary);
#endif
    if (!f.is_open())
      return false;
...
#else
    std::ifstream f((char *)sFile.c_str(), std::ios::in | std::ios::binary);
#endif
    if (!f.is_open())
      return false;
...
int len = 0, done = 0, bits = 0, which = 0, sample_size = 0, position = 0,
    rate = 0;
Sint16 *stream[2];
...
public:
  ...
  int ConstructConsole(int width, int height, int fontw, int fonth) {
    ...
    memset(m_bufScreen[0], 0,
           m_nScreenWidth * m_nScreenHeight * sizeof(CHAR_INFO));
    memset(m_bufScreen[1], 0,
           m_nScreenWidth * m_nScreenHeight * sizeof(CHAR_INFO));
    m_nCurrentBuffer = 0;
  ...
  void Fill(int x1, int y1, int x2, int y2, wchar_t c = PIXEL_SOLID,
            short col = 0x000F) {
    Clip(x1, y1);
    Clip(x2, y2);
    for (int x = x1; x < x2; x++)
  ...
  void DrawLine(int x1, int y1, int x2, int y2, wchar_t c = PIXEL_SOLID,
                short col = 0x000F) {
  ...
  void DrawCircle(int xc, int yc, int r, wchar_t c = PIXEL_SOLID,
                  short col = 0x000F) {
    int x = 0;
    int y = r;
    int p = 3 - 2 * r;
  ...
  void FillCircle(int xc, int yc, int r, wchar_t c = PIXEL_SOLID,
                  short col = 0x000F) {
  ...
  void DrawPartialSprite(int x, int y, olcSprite *sprite, int ox, int oy, int w,
                         int h) {
    if (sprite == nullptr)
      return;
    ...
    for (int i = 0; i < w; i++) {
      for (int j = 0; j < h; j++) {
        if (sprite->GetGlyph(i + ox, j + oy) != L' ')
          Draw(x + i, y + j, sprite->GetGlyph(i + ox, j + oy),
  ...
  void DrawWireFrameModel(
      ...
      float y, float r = 0.0f, float s = 1.0f, short col = FG_WHITE) {
    ...
    for (int i = 0; i < verts + 1; i++) {
      ...
      DrawLine((int)vecTransformedCoordinates[i % verts].first,
               ...
               (int)vecTransformedCoordinates[j % verts].second, PIXEL_SOLID,
               col);
    }
  }
...
private:
  ...
  void GameThread() {
    ...
    // Create Window
    m_window =
        SDL_CreateWindow(bufAppName, SDL_WINDOWPOS_UNDEFINED,
                         SDL_WINDOWPOS_UNDEFINED, m_nScreenWidth * m_nFontWidth,
                         m_nScreenHeight * m_nFontHeight, SDL_WINDOW_SHOWN);

    // Create renderer
    m_render = SDL_CreateRenderer(
        m_window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_TARGETTEXTURE);
    ...
    m_screen = SDL_CreateTexture(
        m_render, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
        m_nScreenWidth * m_nFontWidth, m_nScreenHeight * m_nFontHeight);
    ...
    while (m_bAtomActive) {
      ...
      while (m_bAtomActive) {
        ...
        snprintf(
            s, 256,
            "lubiak.pages.dev - Console Game Engine (SDL) - %s - FPS: %3.2f",
            bufAppName, 1.0f / fElapsedTime);
        ...
        for (int x = 0; x < m_nScreenWidth; x++) {
          for (int y = 0; y < m_nScreenHeight; y++) {
            ...
            if (buff_new[i].colour != buff_old[i].colour ||
                buff_new[i].glyph != buff_old[i].glyph) {
              // Draw Cell
              ...
              SDL_Rect dst = {x * m_nFontWidth, y * m_nFontHeight, m_nFontWidth,
                              m_nFontHeight};
...
protected
  ...
  public:
    ...
    olcAudioSample(std::string sWavFile, olcConsoleGameEngine &cge) {
      ...
      if (!SDL_LoadWAV(sWavFile.c_str(), &fileSpec, (uint8_t **)&wavData,
                       &streamLen)) {
      ...
      SDL_AudioCVT cvt;
      if (!SDL_BuildAudioCVT(&cvt, fileSpec.format, fileSpec.channels,
                             fileSpec.freq, cge.sampleSpec.format,
                             cge.sampleSpec.channels, cge.sampleSpec.freq)) {
    ...
    ~olcAudioSample() { SDL_FreeWAV((uint8_t *)fSample); }
  ...
  static void forwardCallback(void *userdata, uint8_t *byteStream, int len) {
    static_cast<olcConsoleGameEngine *>(userdata)->AudioThread(userdata,
                                                               byteStream, len);
  }

  // Audio thread. This loop responds to requests from the soundcard to fill
  // 'blocks' with audio data. If no requests are available it goes dormant
  // until the sound card is ready for more data. The block is fille by the
  // "user" in some manner and then issued to the soundcard.
  void AudioThread(void *userdata, uint8_t *byteStream, int len) {
    ...
    for (unsigned int n = 0; n < len / sizeof(int16_t); n += spec.channels) {
      ...
      for (unsigned int c = 0; c < spec.channels; c++) {
        int16_t sample =
            (int16_t)(clip(GetMixerOutput(c, fTimeStep * i, fTimeStep), 1.0) *
                      fMaxSample);
        buf[i] = sample;
        i++;
      }
  ...
  virtual float onUserSoundSample(int nChannel, float fGlobalTime,
                                  float fTimeStep) {
    return 0.0f;
  }

  // Overriden by user if they want to manipulate the sound before it is played
  virtual float onUserSoundFilter(int nChannel, float fGlobalTime,
                                  float fSample) {
    return fSample;
  }
  ...
  // takes input from all sound sources for that audio frame. This mixer
  // maintains a list of sound locations for all concurrently playing audio
  // samples. Instead of duplicating audio data, we simply store the fact that a
  // sound sample is in use and an offset into its sample data. As time
  // progresses we update this offset until it is beyound the length of the
  // sound sample it is attached to. At this point we remove the playing souind
  // from the list.
  //
  // Additionally, the users application may want to generate sound instead of
  // just playing audio clips (think a synthesizer for example) in which case we
  // also provide an "onUser..." event to allow the user to return a sound for
  // that point in time.
  //
  // Finally, before the sound is issued to the operating system for performing,
  // the user gets one final chance to "filter" the sound, perhaps changing the
  // volume or adding funky effects
  float GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep) {
    ...
    for (auto &s : listActiveSamples) {
      ...
      if (s.nSamplePosition < vecAudioSamples[s.nAudioSampleID - 1].nSamples)
        fMixerSample +=
            vecAudioSamples[s.nAudioSampleID - 1]
                .fSample[(s.nSamplePosition * spec.channels) + nChannel];
      else
    ...
    listActiveSamples.remove_if(
        [](const sCurrentlyPlayingSample &s) { return s.bFinished; });

    // The users application might be generating sound, so grab that if it
    // exists
